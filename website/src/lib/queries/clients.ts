...
export function getAllClients(businessType?: string): ClientRow[] {
  const db = getDb();
  if (businessType) {
    return db.prepare(`
      SELECT * FROM clients WHERE business_type = ? ORDER BY updated_at DESC
    `).all(businessType) as ClientRow[];
  }
  return db.prepare(`
...
export function searchClients(query: string, businessType?: string): ClientRow[] {
  const db = getDb();
  const pattern = `%${query}%`;
  if (businessType) {
    return db.prepare(`
      SELECT * FROM clients
      WHERE business_type = ?
        AND (q02_client_name LIKE ?
         OR q01_business_name LIKE ?
         OR q03_email LIKE ?
         OR q05_service_type LIKE ?
         OR phone LIKE ?)
      ORDER BY updated_at DESC
    `).all(businessType, pattern, pattern, pattern, pattern, pattern) as ClientRow[];
  }
  return db.prepare(`
    SELECT * FROM clients
...
export function getClientCount(businessType?: string): number {
  const db = getDb();
  if (businessType) {
    const row = db.prepare('SELECT COUNT(*) as count FROM clients WHERE business_type = ?').get(businessType) as { count: number };
    return row.count;
  }
...
export function getClientCountByStatus(businessType?: string): Record<string, number> {
  const db = getDb();
  const rows = businessType
    ? db.prepare(`SELECT status, COUNT(*) as count FROM clients WHERE business_type = ? GROUP BY status`).all(businessType) as Array<{ status: string; count: number }>
    : db.prepare(`SELECT status, COUNT(*) as count FROM clients GROUP BY status`).all() as Array<{ status: string; count: number }>;
...
export function getUpcomingSessions(days: number = 7, businessType?: string): ClientRow[] {
  const db = getDb();
  if (businessType) {
    return db.prepare(`
      SELECT * FROM clients
    ...
    `).all(businessType, days) as ClientRow[];
  }
  return db.prepare(`
    SELECT * FROM clients
...
export function getOverdueFollowups(businessType?: string): ClientRow[] {
  const db = getDb();
  if (businessType) {
    return db.prepare(`
      SELECT * FROM clients
...
export function getMonthlyRevenue(businessType?: string): number {
  const db = getDb();
  if (businessType) {
    const row = db.prepare(`
      SELECT COALESCE(SUM(re.amount), 0) as total FROM revenue_entries re
      JOIN clients c ON re.client_id = c.id
      WHERE re.date >= date('now', 'start of month')
        AND c.business_type = ?
    `).get(businessType) as { total: number };
    return row.total;
  }
  const row = db.prepare(`
...
export function getTestimonialCount(businessType?: string): number {
  const db = getDb();
  if (businessType) {
    const row = db.prepare('SELECT COUNT(*) as count FROM clients WHERE testimonial_received = 1 AND business_type = ?').get(businessType) as { count: number };
    return row.count;
  }
